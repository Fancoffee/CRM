...
class PosConfig(models.Model):
    ...
    pos_credit_note = fields.Boolean('Is Delivery Note')
...
class PosOrder(models.Model):
    ...
    state = fields.Selection(selection_add=[(
        'credit_note', 'Delivery Note'), ('done_credit_note', 'Delivery Note')])
    is_delivery_note = fields.Boolean('Is Delivery Note')

    def write(self, vals):
        for order in self:
            if order.state == 'credit_note' and order.name == '/':
    ...
    def _order_fields(self, ui_order):
        ...
        ui_order_rec = super(PosOrder, self)._order_fields(ui_order)
        if ui_order.get('credit_note'):
            ui_order_rec['state'] = 'credit_note'
            ui_order_rec['is_delivery_note'] = True
        return ui_order_rec

    @api.model
    def _process_order(self, order, draft, existing_order):
        ui_order_rec = super(PosOrder, self)._process_order(
            order, draft, existing_order)
        rec = self.browse(ui_order_rec)
        if rec.state == 'credit_note':
            rec.create_picking()
        return ui_order_rec
                ...
                if order.is_invoiced:
                    ...
                    if self.company_id.anglo_saxon_accounting and order.picking_id.id:
                        ...
                        for move in stock_moves:
                            ...
                            amount = - \
                                sum(move.sudo().stock_valuation_layer_ids.mapped(
                                    'value'))
